...
import { CreateChallengeDto, VerifyChallengeDto } from './dto/create-challenge.dto.js';
...
export class ChallengeController {
  ...
  @ApiResponse({ status: 429, description: 'Throttled or cooldown in progress' })
  async create(
    @ProjectContext('id') projectId: string,
    @ProjectContext('workspaceId') workspaceId: string,
    @Body() createChallengeDto: CreateChallengeDto,
  ) {
    return this.challengeService.createChallenge(projectId, workspaceId, createChallengeDto);
  }
  ...
  @ApiResponse({ status: 400, description: 'Invalid code, expired, or maximum attempts reached' })
  async verify(
    @ProjectContext('id') projectId: string,
    @Param('id') id: string,
    @Body() verifyChallengeDto: VerifyChallengeDto,
  ) {
    return this.challengeService.verifyChallenge(id, projectId, verifyChallengeDto.code);
  }
  ...
  @ApiOperation({ summary: 'Resend OTP code for an active challenge under cooldown rules' })
  ...
  @HttpCode(HttpStatus.OK)
  @RequiredScopes('challenge:verify')
  @ApiOperation({ summary: 'Cancel/invalidate an active verification challenge' })
