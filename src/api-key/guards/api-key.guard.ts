...
export class ApiKeyGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!rawKey) {
      throw new UnauthorizedException('API Key no proporcionada en la cabecera x-api-key.');
    }

    const apiKey = await this.apiKeyService.validateApiKey(rawKey);
    if (!apiKey) {
      throw new UnauthorizedException('API Key inválida, deshabilitada o expirada.');
    }
    ...
    if (requiredScopes && requiredScopes.length > 0) {
      ...
      if (!hasScopes) {
        throw new ForbiddenException('La API Key no cuenta con los scopes requeridos para esta acción.');
      }
    }
